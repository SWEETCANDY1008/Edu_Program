...
const create_file = function() {
	...
	contentFolders.map(lesson => {
		// lesson => 각각의 대단원들
		// lesson == 1, 2, 3, 4, .....
		const content = fs.readdirSync('./contents/lesson/' + lesson + '/chapter/');
		...
		content.map(chapter => {
			...
			const body = fs.readFileSync('./contents/lesson/' + lesson + '/chapter/' + chapter + '/content.md', "utf8");
			
			// markdown파일을 렌더링
			const convertedBody = md.render(body);
			
			// 해당 대단원의 chapter안에 있는 내용들의 목록
			const filelist = fs.readdirSync('./contents/lesson/' + lesson + '/chapter/' + chapter);
			
			// 만약 그 목록중 code란 이름의 파일이 없는경우
			if(filelist.indexOf("code") === -1) {
				// 오른쪽칸에 컴파일러 대신 원하는 내용을 띄울 수 있음
				const test = fs.readFileSync("./views/part/template/test.ejs", "utf8");
				var judgment_compiler = ejs.render(test);
			} else {
				// code란 파일이 존재하는경우 오른쪽칸에 컴파일러를 띄운다.
				const code = fs.readFileSync('./contents/lesson/' + lesson + '/chapter/' + chapter + '/code', "utf8");
			...
			const fileName = getHtmlFileName(chapter);
			if (!fs.existsSync(dir + '/' + lesson)) {
				fs.mkdirSync(dir + '/' + lesson);
			}
			
			// 파일을 작성한다.
			fs.writeFileSync(`./views/part/section/${lesson}/${chapter}.ejs`, articleContent);
		});
	});
};
...
// 컴파일러가 있는 페이지에서 RUN버튼을 눌렀을때 ajax형태로 post를 받았을때 접근
app.post("/running", function(request, response) {
	// 받은 정보를 바탕으로 서버에서 python파일을 실행시킨 결과를 받음
	var result = py_compile(request, response);
	console.log('POST Parameter = ' + result);
	// 이것을 다시 ajax를 통해 전송
...
const py_compile = function(request, response) {
